...
import { crm, type SearchHit, type TimelineOut } from '@/api/crm';
...
export function TopBar() {
  ...
  const [recent, setRecent] = useState<TimelineOut[] | null>(null);
    ...
    <header className="flex h-14 shrink-0 items-center gap-3 border-b border-border bg-card px-4">
      ...
      <Dialog open={open} onOpenChange={(o) => { setOpen(o); if (!o) setQ(''); }}>
        <DialogContent className="overflow-hidden p-0 sm:max-w-[560px]">
          ...
          {/* Server-side search: results are already ranked, so cmdk must not re-filter by its own value matching. */}
        ...
        <CommandList>
          ...
          {hits.filter((h) => h.type === 'client').length > 0 && (
            <CommandGroup heading="Clients">{hits.filter((h) => h.type === 'client').map((h) => <CommandItem key={h.id} value={h.id} onSelect={() => go(`/clients/${h.client_id}`)}><Building2 className="mr-2 size-4 text-muted-foreground" /> {h.label}<span className="ml-auto text-[12px] text-muted-foreground">{h.sublabel}</span></CommandItem>)}</CommandGroup>
          )}
          {hits.filter((h) => h.type === 'contact').length > 0 && (
            <CommandGroup heading="Contacts">{hits.filter((h) => h.type === 'contact').map((h) => <CommandItem key={h.id} value={h.id} onSelect={() => go(`/clients/${h.client_id}`)}><UserIcon className="mr-2 size-4 text-muted-foreground" /> {h.label}<span className="ml-auto text-[12px] text-muted-foreground">{h.sublabel}</span></CommandItem>)}</CommandGroup>
          )}
      ...
      <Popover onOpenChange={(o) => { if (o) void crm.home().then((h) => setRecent(h.recent)).catch(() => setRecent([])); }}>
        <PopoverTrigger asChild><Button variant="ghost" size="icon" aria-label="Notifications"><Bell className="size-4" /></Button></PopoverTrigger>
        <PopoverContent align="end" className="w-[360px] p-0">
          <div className="border-b px-4 py-2.5 text-[13px] font-semibold">Recent activity</div>
          <ul className="max-h-[360px] overflow-y-auto">
            {recent?.map((a) => { const m = getModule(a.module_key); return (
              <li key={a.id} className="flex gap-3 border-b px-4 py-3 last:border-0 hover:bg-secondary">
                ...
                <div className="min-w-0"><div className="truncate text-[13px]">{a.summary}</div><div className="text-[12px] text-muted-foreground">{a.client_name ?? m.shortName} · {formatDistanceToNow(new Date(a.occurred_at), { addSuffix: true })}</div></div>
              </li>
            ); })}
            {recent && recent.length === 0 && <li className="px-4 py-8 text-center text-[13px] text-muted-foreground">Nothing yet.</li>}
            {!recent && <li className="px-4 py-8 text-center text-[13px] text-muted-foreground">Loading…</li>}
          </ul>
        </PopoverContent>
      </Popover>
